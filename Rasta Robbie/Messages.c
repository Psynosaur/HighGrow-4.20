...
int RobbieWalk(HWND hWnd, POINT ptStart, POINT ptEnd, int iPose)
   {
   int i;
   int iDistance=0;  // pixels covered in this walk
   int dx, dy;       // holds the calculated delta x and y

   i     = anim_init(ptStart, ptEnd, 20, &sprite_01, &hRobbieBmps[0], iPose);
   ...
   iDistance = (int)max((sqrtl(powl(dx,2)+powl(dy,2))), 1.0);
   ...
   anim_animate(hWnd, WM_ENDFRAME, gnWalkSpeed);
   return iDistance;
   }
...
int PlayRobbieFrame(HWND hwnd)
   ...
   // set the walking going...
   iPaces = RobbieWalk(hwnd, ptStart, ptEnd, FrameToonStep.itPose);
   gbFrameWalking = TRUE;
...
void FramePlaybackTimer(HWND hWnd)
   ...
   if(gbSingleWalk)
      ...
      if(FrameToonStep.itXpos<((rc.right-rc.left)/2))
         ...
         FrameToonStep.itXpos = (rc.right-rc.left)+50; 
...
int RobbiePlayToonStep(HWND hWnd, PTOONSTEP tstep, POINT ptEnd)
...
int RobbiePlayToonFrame(HWND hWnd, POINT ptEnd, int iPose, BOOL bWink, LPSTR szText)
   {
   if(gbFrameWalking) return FALSE;
